...
// ----------------------------------------------------------------------
// Modified for Okta Sign-in and Sign-out flows
// Last Modified: 12/13/2017 by Sami
// ----------------------------------------------------------------------
var AuthenticationContext = (function () {
    ...
    AuthenticationContext = function (config) {
        ...
        //-------------------------------------------------------------------------------------------------
        // Modified for Okta User Authentication
        //-------------------------------------------------------------------------------------------------
        // For Okta logout id_token is required
        this.idToken = null;
  ...
  */
    ...
    AuthenticationContext.prototype.logOut = function () {
        ...
        if (this.config.logOutUri) {
            ...
            //-------------------------------------------------------------------------------------------------
            // Modified for Okta Logout
            //-------------------------------------------------------------------------------------------------
            //urlNavigate = this.instance + tenant + '/oauth2/logout?' + logout;

            // For Okta Logout, id_token_hint and post_logout_redirect_uri are required parameters
            urlNavigate = this.instance + 'logout?post_logout_redirect_uri=' + encodeURIComponent(this.config.postLogoutRedirectUri) + '&id_token_hint=' + this.idToken;
            //-------------------------------------------------------------------------------------------------
            // Last Modified: 12/13/2017 by Sami
            //-------------------------------------------------------------------------------------------------
        }
        ...
        if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
            ...
            if (requestInfo.stateMatch) {
                ...
                if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                    ...
                    // For Okta Logout, id_token is a required parameter
                    this.idToken = requestInfo.parameters[this.CONSTANTS.ID_TOKEN];
    ...
    AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource) {
        ...
        var urlNavigate = this.instance + 'authorize' + this._serialize(responseType, this.config, resource) + this._addLibMetadata();
    ...
    AuthenticationContext.prototype._addLibMetadata = function () {
        ...
        if (this.config.tenant) {
            return '&scope=openid+profile&idp=' + this.config.tenant + '&x-client-SKU=Js&x-client-Ver=' + this._libVersion();
        } else {
            return '&scope=openid+profile&x-client-SKU=Js&x-client-Ver=' + this._libVersion();
        }
